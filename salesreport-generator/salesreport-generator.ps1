...
"@
    ...
    $outputFileName = "salesreport-return_${channelSign}_${timestamp}.csv"
    ...
    function GetCSVOutputString($item, $stockId, $stockName, $sourceId, $sourceName)
	{
    ...
    if (($items.Count -ge 1) -and ($selectedStockLocationName1) -and ($selectedSourceLocationName1)) {

        $output += GetCSVOutputString $items[0] $stockLocationId1 $selectedStockLocationName1 $sourceLocationId1 $selectedSourceLocationName1
    }
    # work on the optional parameters if order has more than one item and locations are selected on TeamCity
   
    if (($items.Count -ge 2) -and ($selectedStockLocationName2) -and ($selectedSourceLocationName2))
        {
            ...
            $output += GetCSVOutputString $order.ITEMS.ITEM[1] $stockLocationId2 $selectedStockLocationName2 $sourceLocationId2 $selectedSourceLocationName2
        ...
        if (($selectedStockLocationName3) -and ($selectedSourceLocationName3))
            {
                ...
                $output += GetCSVOutputString $order.ITEMS.ITEM[2] $stockLocationId3 $selectedStockLocationName3 $sourceLocationId3 $selectedSourceLocationName3 
            }
